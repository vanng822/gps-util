...
var tcxParse = function(data, callback) {
	parser.parseString(data, function(err, result) {
		...
		if(result.TrainingCenterDatabase && result.TrainingCenterDatabase.Activities) {
			...
			callback(new Error('Unexpected data'), null);
		}
	});
};
...
var tcxParseURL = function(url, callback) {
			...
			if(res.statusCode == 200) {
				...
				magic.detect(buf, function(err, result) {
					if(result == 'application/xml' || result == 'text/xml') {
						tcxParse(buf, callback);
					} else {
						callback(new Error('Got unexpected data type'), null);
					}
				});
			} else {
				callback(new Error('Got unexpected response code'), null);
			}
		});
