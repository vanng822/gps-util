...
vows.describe('Test suite for calc').addBatch({
	'Calculate speed and distance for gpx' : {
		...
		'expected result from calculateFromGPX' : function(err, result) {
			...
			assert.ok(Math.abs(result.fastestSpeed - 6.96311967571195) < preci);
			assert.ok(Math.abs(result.slowestSpeed - 1.7153931748868099) < preci);
			...
			assert.ok(Math.abs(point2.speed - 4.221760443523958) < preci);
			...
			assert.ok(Math.abs(point18.speed - 4.084766149733331) < preci);
			assert.ok(Math.abs(point18.distance - 393.5258418687884) < preci);
